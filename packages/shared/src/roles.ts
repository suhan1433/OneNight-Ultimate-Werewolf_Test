...
const role = (id: RoleType, name: string, emoji: string, faction: RoleDefinition['faction'], nightOrder: number | null, nightAction: RoleDefinition['nightAction'], description: string, winCondition = villageWin, maxCount = 1): RoleDefinition =>
  ({ id, name, emoji, faction, nightOrder, nightAction, description, winCondition, canActAtNight: nightOrder !== null, maxCount });

export const ROLE_DEFINITIONS: Record<RoleType, RoleDefinition> = {
  doppelganger: role('doppelganger','도플갱어','🪞','village',1,'inspect_player','한 명의 역할을 복사하고 그 능력을 즉시 사용합니다.','복사한 역할의 승리 조건을 따릅니다.'),
  shield_bearer: role('shield_bearer','방패병','🛡️','village',2,'protect','한 명을 보호합니다.'),
  ...
  secret_agent: role('secret_agent','비밀 요원','🕵️','village',8,'confirm','다른 비밀 요원을 확인합니다.'),
  ...
  mason: role('mason','프리메이슨','🗿','village',null,'none','다른 프리메이슨과 서로를 압니다.',villageWin,2),
...
export const WOLF_ROLES: RoleType[] = ['werewolf','alpha_wolf','mystic_wolf','dream_wolf'];
export const PRESETS: Record<string, RoleType[]> = {
