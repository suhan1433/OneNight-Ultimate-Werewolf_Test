...
import { NIGHT_ROLES, ROLE_DEFINITIONS, WOLF_ROLES } from '@werewolf/shared';
export function shuffleRoles(items, random = Math.random) {
    const result = [...items];
...
export function assignRoles(players, selectedRoles, random = Math.random) {
...
export function buildNightActionQueue(selectedRoles, players) {
...
export function startCurrentAction(room, now = Date.now()) {
...
export function advanceNight(room, status, now = Date.now()) {
    ...
    if (action)
        queue[room.currentNightActionIndex] = { ...action, status };
...
export function validateNightAction(room, playerId, actionId, command) {
    if (room.phase !== 'night')
        return '현재 밤 페이즈가 아닙니다.';
    const action = room.nightActionQueue[room.currentNightActionIndex];
    if (!action || action.id !== actionId || action.status !== 'active')
        return '이미 끝났거나 현재 행동이 아닙니다.';
    if (!action.playerIds.includes(playerId) || action.actedPlayerIds.includes(playerId))
        return '현재는 당신의 차례가 아닙니다.';
    const self = room.players.find((p) => p.id === playerId);
    const targets = command.targetPlayerIds ?? [];
    if (targets.some((id) => id === playerId || !room.players.some((p) => p.id === id && p.connected)))
        return '유효하지 않은 대상입니다.';
    if (room.protectedPlayerId && targets.includes(room.protectedPlayerId))
        return '방패병이 보호한 카드는 확인하거나 바꿀 수 없습니다.';
    if (new Set(targets).size !== targets.length)
        return '서로 다른 대상을 선택하세요.';
    if ((command.centerIndexes ?? []).some((i) => i < 0 || i >= 3))
        return '유효하지 않은 센터 카드입니다.';
    ...
    if (exactPlayers !== undefined && targets.length !== exactPlayers)
        return `${exactPlayers}명의 플레이어를 선택하세요.`;
    if (role === 'seer' && !((targets.length === 1 && !command.centerIndexes?.length) || (targets.length === 0 && command.centerIndexes?.length === 2)))
        return '플레이어 1명 또는 센터 카드 2장을 선택하세요.';
    if (role === 'apprentice_seer' && command.centerIndexes?.length !== 1)
        return '센터 카드 1장을 선택하세요.';
    if (role === 'drunk' && command.centerIndexes?.length !== 1)
        return '센터 카드 1장을 선택하세요.';
    if (role === 'witch') {
    ...
    if (role === 'werewolf') {
        ...
        if (!wolves.length && command.type !== 'inspect_center')
            return '혼자인 늑대는 센터 카드 한 장을 확인하세요.';
        if (!wolves.length && command.centerIndexes?.length !== 1)
            return '센터 카드 한 장을 선택하세요.';
    }
    return null;
}
export function applyNightAction(room, playerId, command) {
    const action = room.nightActionQueue[room.currentNightActionIndex];
    ...
    let result = {};
    const shielded = (p) => p?.id === room.protectedPlayerId;
    switch (action.role) {
        case 'doppelganger': {
            if (!target || shielded(target))
                break;
            self.currentRole = target.currentRole;
            result = { kind: 'copied', targetNickname: target.nickname, role: target.currentRole };
            const copied = target.currentRole && ROLE_DEFINITIONS[target.currentRole];
            if (copied?.canActAtNight && copied.id !== 'doppelganger') {
                const follow = { id: randomUUID(), role: copied.id, playerIds: [self.id], actedPlayerIds: [], order: action.order + 0.1, startedAt: 0, expiresAt: 0, status: 'pending', copied: true };
        ...
        case 'shield_bearer':
            room = { ...room, protectedPlayerId: target?.id ?? null };
            result = { kind: 'protected', nickname: target?.nickname };
            break;
        case 'werewolf': {
        ...
        case 'alpha_wolf':
            if (target && !shielded(target) && !(target.currentRole && WOLF_ROLES.includes(target.currentRole))) {
                target.currentRole = 'werewolf';
                result = { kind: 'alpha_swap', targetNickname: target.nickname };
            }
            break;
        case 'mystic_wolf':
        case 'journalist':
            if (target && !shielded(target)) {
                result = { kind: 'cards', title: action.role === 'mystic_wolf' ? '신비한 늑대가 확인한 카드' : '신문기자가 확인한 카드', cards: [{ label: target.nickname, role: target.currentRole }] };
                if (action.role === 'journalist' && target.currentRole && ROLE_DEFINITIONS[target.currentRole].faction === 'village')
                    room = { ...room, publicReveals: [...room.publicReveals, `${target.nickname}님은 마을 진영입니다.`] };
            }
            break;
        case 'minion':
            result = { kind: 'people', title: '늑대인간', people: players.filter((p) => p.currentRole && WOLF_ROLES.includes(p.currentRole)).map(privateRole) };
            break;
        ...
        case 'seer':
            if (target && !shielded(target))
                ...
                result = { kind: 'cards', title: '예언자가 확인한 센터 카드', cards: indexes.map((i) => ({ label: `센터 카드 ${i + 1}`, role: centers[i].role })) };
            break;
        case 'apprentice_seer':
            result = { kind: 'cards', title: '견습 예언자가 확인한 센터 카드', cards: indexes.map((i) => ({ label: `센터 카드 ${i + 1}`, role: centers[i].role })) };
            break;
        case 'robber':
            if (target && !shielded(target)) {
                const myOldRole = self.currentRole;
                const targetOldRole = target.currentRole;
                [self.currentRole, target.currentRole] = [targetOldRole, myOldRole];
                result = { kind: 'robber_swap', targetNickname: target.nickname, myNewRole: self.currentRole, targetNewRole: target.currentRole };
            }
            break;
        case 'witch': {
            const center = centers[indexes[0]];
            if (!target) {
                result = { kind: 'witch_seen', centerIndex: indexes[0], seenRole: center.role };
                break;
            }
            if (!shielded(target)) {
                const seenRole = center.role;
                [center.role, target.currentRole] = [target.currentRole, center.role];
                result = { kind: 'witch_swap', targetNickname: target.nickname, seenRole };
            }
            break;
        }
        case 'troublemaker':
            if (target && target2 && !shielded(target) && !shielded(target2)) {
                [target.currentRole, target2.currentRole] = [target2.currentRole, target.currentRole];
                result = { kind: 'troublemaker_swap', firstNickname: target.nickname, secondNickname: target2.nickname };
            }
            break;
        case 'drunk': {
            const center = centers[indexes[0]];
            [center.role, self.currentRole] = [self.currentRole, center.role];
            result = { kind: 'drunk_swap', centerIndex: indexes[0] + 1 };
            break;
        }
        case 'insomniac':
            result = { kind: 'cards', title: '불면증 환자의 현재 카드', cards: [{ label: '내 카드', role: self.currentRole }] };
            break;
...
const privateRole = (p) => ({ nickname: p.nickname });
export function startDay(room, now = Date.now()) {
    return { ...room, phase: 'day', dayExpiresAt: now + room.dayTimeLimitSeconds * 1000, updatedAt: now };
}
export function calculateVotes(room) {
...
export function calculateResult(room) {
    ...
    for (const hunter of players.filter((p) => p.currentRole === 'hunter' && executed.has(p.id))) {
        const target = room.votes[hunter.id];
        if (target)
            executed.add(target);
    }
...
export function buildPlayerGameState(room, playerId, actionResults = {}) {
    ...
    if (isActor && action) {
        ...
        if (action.role === 'insomniac')
            actionContext = { kind: 'cards', title: '내 현재 카드', cards: [{ label: '내 카드', role: self.currentRole }] };
    }
    return {
        ...
        isNightActor: isActor, actionContext, actionResult: actionResults[playerId], votesCompleted: Object.keys(room.votes).length, totalPlayers: room.players.length,
        dayExpiresAt: room.dayExpiresAt, chat: room.chat.slice(-100), publicReveals: room.publicReveals,
