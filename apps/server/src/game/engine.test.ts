...
describe('night actions',()=>{
  ...
  it('witch first sees a center card, then swaps it with the chosen player',()=>{const r=actionRoom('witch',[player('마법사','witch'),player('철수','robber'),player('영희')]);expect(validateNightAction(r,'마법사','action',{type:'inspect_center',centerIndexes:[0]})).toBeNull();const seen=applyNightAction(r,'마법사',{type:'inspect_center',centerIndexes:[0]});expect(seen.result).toEqual({kind:'witch_seen',centerIndex:0,seenRole:'tanner'});expect(seen.room.nightActionQueue[0]!.actedPlayerIds).toEqual([]);const next=applyNightAction(seen.room,'마법사',{type:'swap_center',targetPlayerIds:['철수'],centerIndexes:[0]});expect(next.room.centerCards[0]!.role).toBe('robber');expect(next.room.players[1]!.currentRole).toBe('tanner');expect(next.result).toMatchObject({kind:'witch_swap',seenRole:'tanner',targetNickname:'철수'});});
  ...
  it('werewolf, minion, apprentice tanner, and secret agent expose only the relevant people',()=>{
    ...
    const apprentice=applyNightAction(actionRoom('apprentice_tanner',[player('견습','apprentice_tanner'),player('무두','tanner'),player('요원','secret_agent')]),'견습',{type:'confirm'}).result;
    const agent=applyNightAction(actionRoom('secret_agent',[player('요원1','secret_agent'),player('요원2','secret_agent'),player('영희')]),'요원1',{type:'confirm'}).result;
    expect(wolf).toMatchObject({kind:'people',people:[{nickname:'늑대2'}]});
    expect(apprentice).toMatchObject({kind:'people',people:[{nickname:'무두'}]});
    expect(agent).toMatchObject({kind:'people',people:[{nickname:'요원2'}]});
  });
