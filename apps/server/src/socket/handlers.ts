...
export function registerHandlers(io: Server, socket: Socket) {
  ...
  socket.on('ROOM_SYNC', async (callback?: (ack: Ack<ClientGameState>) => void) => {
    ...
    if (!code || !playerId) { callback?.({ ok: false, error: '유효한 게임 세션이 없습니다.' }); return; }
    ...
    try {
      ...
      if (!room || !player?.connected || player.socketId !== socket.id) { callback?.({ ok: false, error: '게임 상태를 찾을 수 없습니다.' }); return; }
  ...
  const relayVoiceSignal = (event: 'VOICE_OFFER' | 'VOICE_ANSWER' | 'VOICE_ICE') => on(socket, event, async (raw) => {
    ...
    if (event === 'VOICE_ICE') { if (socket.data.voiceRoomCode !== data.roomCode) throw new Error('음성 대화에 참가해주세요.'); io.to(playerChannel(data.targetPlayerId)).emit(event, { senderId: playerId, payload: data.payload }); return {}; }
    const room = await getRoom(data.roomCode);
    if (!room || !['lobby', 'day'].includes(room.phase) || !room.players.some((player) => player.id === data.targetPlayerId && player.socketId && player.connected)) throw new Error('현재 음성 연결을 만들 수 없습니다.');
  ...
  on(socket, 'ROOM_JOIN', async (raw) => {
    ...
    const room = await withRoomLock(data.roomCode, async (room) => {
      ...
      if (reconnect) { reconnect.socketId = socket.id; reconnect.connected = true; reconnect.disconnectedAt = null; room.allOfflineExpiresAt = null; joinedId = reconnect.id; joinedToken = reconnect.sessionToken; }
        ...
        if (room.moderatorMode) throw new Error('오프라인 사회자 방에는 다른 기기로 참가할 수 없습니다.');
  ...
  on(socket, 'GAME_START', async (raw) => mutate(io, socket, raw, async (room, playerId) => {
    ...
    await clearLobbyChat(room.roomCode);
  }, 'GAME_STARTED', (room) => { if (room.moderatorMode) io.to(roomChannel(room.roomCode)).emit('NARRATOR_SPEECH', { text: NARRATOR_LINES.nightStart, audioKey: 'night-start', actionId: 'night-start', stateVersion: room.updatedAt, timestamp: Date.now() }); }));
  ...
  }, 'CARD_CONFIRM_PROGRESS', (room) => { if (room.phase === 'night') io.to(roomChannel(room.roomCode)).emit('NARRATOR_SPEECH', { text: NARRATOR_LINES.nightStart, audioKey: 'night-start', actionId: 'night-start', stateVersion: room.updatedAt, timestamp: Date.now() }); }));
  on(socket, 'NIGHT_ACTION_SUBMIT', async (raw) => {
    ...
    const room = await withRoomLock(base.roomCode, async (room, alreadyProcessed) => {
      if (alreadyProcessed) { duplicate = true; return room; }
  ...
  on(socket, 'DAY_START', async (raw) => mutate(io, socket, raw, (room, playerId) => {
    ...
    if (room.moderatorMode) throw new Error('오프라인 사회자 모드는 투표를 진행하지 않습니다.');
  ...
  on(socket, 'VOTE_CONFIRM', async (raw) => {
    const data = safe(requestSchema.extend({ targetPlayerId: z.string().uuid() }), raw); const playerId = assertSession(socket, data.roomCode);
    const room = await getRoom(data.roomCode); const target = data.targetPlayerId;
    if (!room || room.phase !== 'voting') throw new Error('투표 단계가 아닙니다.');
...
export function emitDayStart(io: Server, room: Room) { io.to(roomChannel(room.roomCode)).emit('NARRATOR_SPEECH', { text: NARRATOR_LINES.dayStart, audioKey: 'day-start', actionId: 'day-start', stateVersion: room.updatedAt, timestamp: Date.now() }); io.to(roomChannel(room.roomCode)).emit('DAY_STARTED', { expiresAt: room.dayExpiresAt }); }
