...
interface Store { game: ClientGameState | null; tts: boolean; help: boolean; error: string | null; setGame: (game: ClientGameState | null) => void; setChatHistory: (messages: ChatMessage[]) => void; addChat: (message: ChatMessage) => void; removeChat: (id: string) => void; setTts: (tts: boolean) => void; setHelp: (help: boolean) => void; setError: (error: string | null) => void; }
...
}), removeChat: (id) => set((state) => !state.game ? state : ({ game: { ...state.game, chat: state.game.chat.filter((message) => message.id !== id) } })), setTts: (tts) => { localStorage.setItem('tts', tts ? 'on' : 'off'); set({ tts }); }, setHelp: (help) => set({ help }), setError: (error) => set({ error }) }));
