...
interface Store { game: ClientGameState | null; tts: boolean; help: boolean; error: string | null; connectionState: 'connected' | 'reconnecting'; transitioningActionId: string | null; setGame: (game: ClientGameState | null) => void; setChatHistory: (messages: ChatMessage[]) => void; setLobbyChatHistory: (messages: ChatMessage[]) => void; addChat: (message: ChatMessage, scope: ChatScope) => void; removeChat: (id: string, scope: ChatScope) => void; toggleReady: () => void; confirmVote: () => void; rollback: (game: ClientGameState) => void; setTts: (tts: boolean) => void; setHelp: (help: boolean) => void; setError: (error: string | null) => void; setConnectionState: (connectionState: Store['connectionState']) => void; setTransitioningActionId: (actionId: string | null) => void; }
export const useGame = create<Store>((set) => ({ game: null, tts: localStorage.getItem('tts') !== 'off', help: false, error: null, connectionState: 'connected', transitioningActionId: null, setGame: (game) => set((state) => {
  ...
  if (!game || !state.game || game.roomCode !== state.game.roomCode) return { game, transitioningActionId: null };
  if (game.stateVersion < state.game.stateVersion) return state;
  if (game.stateVersion === state.game.stateVersion) return state;
  const transitioningActionId = state.transitioningActionId && game.currentNightAction?.id === state.transitioningActionId ? state.transitioningActionId : null;
  if (game.phase === 'day' && state.game.phase === 'day') {
    ...
    return { game: { ...game, chat }, transitioningActionId };
  }
  if (game.phase === 'lobby' && state.game.phase === 'lobby') {
    ...
    return { game: { ...game, lobbyChat }, transitioningActionId };
  }
  return { game, transitioningActionId };
