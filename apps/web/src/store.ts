...
interface Store { game: ClientGameState | null; tts: boolean; help: boolean; error: string | null; setGame: (game: ClientGameState | null) => void; setChatHistory: (messages: ChatMessage[]) => void; addChat: (message: ChatMessage) => void; setTts: (tts: boolean) => void; setHelp: (help: boolean) => void; setError: (error: string | null) => void; }
...
}), setTts: (tts) => { localStorage.setItem('tts', tts ? 'on' : 'off'); set({ tts }); }, setHelp: (help) => set({ help }), setError: (error) => set({ error }) }));
